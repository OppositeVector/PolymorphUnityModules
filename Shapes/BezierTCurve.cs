...
namespace Polymorph.Shapes {
    ...
    public class BezierTCurve {
        ...
        public BezierTCurve() { }
        public BezierTCurve(Vector3 start, Vector3 end, Vector3 control1 = default(Vector3), Vector3 control2 = default(Vector3)) {
            var d = (end - start) / 2;
            this.start = new ControledPoint(start, start - d, control1);
            this.end = new ControledPoint(end, control2, end + d);
            length = GetLength();
        }
        public BezierTCurve(ControledPoint start, ControledPoint end) {
            this.start = start;
            this.end = end;
            length = GetLength();
        }

        public Vector3 this[double t] {
            get { return Bezier.GetPoint(start.v, end.v, start.sc, end.pc, t); }
        }
        ...
        public Vector3 GetVelocity(double t) {
            var retVal = Bezier.GetFirstDerivative(start.v, end.v, start.sc, end.pc, t);   
            return retVal;
        }
